...
# 4. 強制更新範本 (確保納入 insecure 選項與正確的 https url)
if [ -f "$CONFIG_FILE" ]; then
    echo "[Info] Updating configuration.yml to satisfy secure scheme requirements..."
...
session:
  cookies:
    - domain: "$DOMAIN"
      authelia_url: "https://auth.$DOMAIN"  # 必須是 https，否則報錯 
